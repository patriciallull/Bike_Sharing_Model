...
def get_data_directory():
    """
    Retrieve Data from the directory where it is located
    """
...
def get_model_path(model_dir = None):#set default to none in case it does not exist
...
def get_season(date):
    date = date.replace(year=2000) # we will use the equinox/solstices

    if SPRING_EQUINOX <= date < SUMMER_SOLSTICE:
        return 2
    elif SUMMER_SOLSTICE <= date < FALL_EQUINOX:
        return 3
    elif FALL_EQUINOX y<= date < WINTER_SOLSTICE:
        return 4
    else:
        return 1
